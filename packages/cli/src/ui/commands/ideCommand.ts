...
import {
  CANOPY_CODE_COMPANION_EXTENSION_NAME,
  getIdeInstaller,
  IDEConnectionStatus,
  ideContextStore,
} from '@canopy-code/canopy-code-core';
    ...
    action: async (context) => {
      ...
      if (!installer) {
        ...
        context.ui.addItem(
          {
            type: 'error',
            text: `Automatic installation is not supported for ${ideName}. Please install the '${CANOPY_CODE_COMPANION_EXTENSION_NAME}' extension manually from the marketplace.`,
          },
          Date.now(),
        );
        return;
