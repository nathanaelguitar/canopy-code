...
\`\`\`bash
"\${CANOPY_CODE_CLI:-canopy}" review base-tree --plan <the plan report> --worktree <this worktree> \\
  --out <the plan report's directory>/canopy-review-pr-<n>-base-tree.json
\`\`\`
...
\`\`\`bash
"\${CANOPY_CODE_CLI:-canopy}" review extract-step --workflow <path in the tree being reviewed> \\
...
\`\`\`bash
"\${CANOPY_CODE_CLI:-canopy}" review mock-provider --responder <a module you write> \\
...
"\${CANOPY_CODE_CLI:-canopy}" review drive --cwd <the worktree> --script <what to run> \\
