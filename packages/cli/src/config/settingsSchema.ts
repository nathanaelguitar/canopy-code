  ...
  model: {
    ...
    properties: {
      ...
      skipLoopDetection: {
        ...
        default: true,
        description:
          'Disable all loop-detection guardrails, including content/thought heuristics, duplicate/stagnant tool-call checks, per-turn caps, and repeated invalid-parameter stops. Defaults to true so long-running agents can recover from tool errors and intentionally repeat calls; tool errors remain visible in the transcript.',
        showInDialog: false,
      },
      maxToolCallsPerTurn: {
        type: 'integer',
        ...
        description:
          'Per-turn tool-call cap (one model turn plus its tool-result continuations). Set to 0 or a negative value to disable the cap. This setting is ignored when model.skipLoopDetection is true (the default), because skipLoopDetection is the master opt-out for all loop-protection stops.',
        showInDialog: false,
      },
      skipStartupContext: {
        type: 'boolean',
