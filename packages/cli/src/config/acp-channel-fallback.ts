 ...
 * `canopy --acp` child reports `channel=daemon` instead of the generic `ACP`.
 * The Tauri desktop shell launches `canopy serve` with `CANOPY_CODE_DESKTOP=1`
 ...
 *
 * A `canopy --acp` child cannot otherwise tell it was daemon-spawned: direct
 * ACP integrations (VS Code companion, Electron desktop, third parties)
 * spawn the same command line.
 */

export const CANOPY_CODE_SERVE_ENV = 'QWEN_CODE_SERVE';
export const CANOPY_CODE_DESKTOP_ENV = 'CANOPY_CODE_DESKTOP';
...
export function resolveAcpChannelFallback(
  ...
  if (env[CANOPY_CODE_DESKTOP_ENV] === '1') {
    return 'desktop';
  }
  if (env[CANOPY_CODE_SERVE_ENV] === '1') {
    return 'daemon';
  }
  return 'ACP';
}
