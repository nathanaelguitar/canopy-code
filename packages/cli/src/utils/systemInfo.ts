 ...
 * For bug reports, removes 'canopy-' or 'canopy-code-' prefixes from sandbox names.
 *
 * @param stripPrefix - Whether to strip 'canopy-' prefix (used for bug reports)
 */
export function getSandboxEnv(stripPrefix = false): string {
  ...
  // For bug reports, remove canopy- prefix
  if (stripPrefix) {
    return sandbox.replace(/^canopy-(?:code-)?/, '');
  }
