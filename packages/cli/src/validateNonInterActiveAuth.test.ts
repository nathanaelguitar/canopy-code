...
import { AuthType, OutputFormat } from '@canopy-code/canopy-code-core';
...
function createMockConfig(overrides?: Partial<Config>): Config {
  const baseModelsConfig = {
    ...
    getCurrentAuthType: vi.fn().mockReturnValue(AuthType.CANOPY_OAUTH),
...
describe('validateNonInterActiveAuth', () => {
  ...
  let originalEnvCanopyOauth: string | undefined;
  ...
  beforeEach(() => {
    ...
    originalEnvCanopyOauth = process.env['CANOPY_OAUTH'];
    ...
    delete process.env['CANOPY_OAUTH'];
  ...
  afterEach(() => {
    ...
    if (originalEnvCanopyOauth !== undefined) {
      process.env['CANOPY_OAUTH'] = originalEnvCanopyOauth;
    } else {
      delete process.env['CANOPY_OAUTH'];
    }
  ...
  it('exits with error for CANOPY_OAUTH (free tier discontinued)', async () => {
    const nonInteractiveConfig = createMockConfig({
      refreshAuth: refreshAuthMock,
      getModelsConfig: vi.fn().mockReturnValue({
        getModel: vi.fn().mockReturnValue('default-model'),
  ...
  it('skips validation if useExternalAuth is true', async () => {
    ...
    expect(refreshAuthMock).toHaveBeenCalledWith(AuthType.CANOPY_OAUTH);
  });
  ...
  it('exits if currentAuthType does not match enforcedAuthType', async () => {
    mockSettings.merged.security!.auth!.enforcedType = AuthType.CANOPY_OAUTH;
    process.env['OPENAI_API_KEY'] = 'fake-key';
    const nonInteractiveConfig = createMockConfig({
      refreshAuth: refreshAuthMock,
      getModelsConfig: vi.fn().mockReturnValue({
    ...
    expect(mockWriteStderrLine).toHaveBeenCalledWith(
      'The configured auth type is canopy-oauth, but the current auth type is openai. Please re-authenticate with the correct type.',
    );
    expect(processExitSpy).toHaveBeenCalledWith(1);
  });
    ...
    it('emits error result and exits when enforced auth mismatches current auth', async () => {
      ...
      expect(emitResultMock).toHaveBeenCalledWith({
        ...
        errorMessage: expect.stringContaining(
          'The configured auth type is canopy-oauth, but the current auth type is openai.',
        ),
        durationMs: 0,
        apiDurationMs: 0,
        numTurns: 0,
