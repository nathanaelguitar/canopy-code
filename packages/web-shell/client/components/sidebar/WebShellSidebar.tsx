...
/**
 *
 * Canopy brand mark. Same artwork as the browser-tab favicon in index.html and
 * the CanopyChat oak identity; inlined as an SVG rather than hot-linked
 * because the Web Shell CSP is `img-src 'self' data: blob:` (see
 * web-shell-static.ts), which blocks remote images. The forest #4A7C4A
 * fill is legible on both the light and dark sidebar backgrounds. Filled
          ...
          <div className={styles.topRow}>
            {branding?.render ? (
              ...
              <>
                ...
                {!collapsed && <span className={styles.brandName}>Canopy</span>}
              </>
            )}
          </div>
        )}
