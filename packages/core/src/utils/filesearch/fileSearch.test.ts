...
describe('FileSearch', () => {
  ...
  it('should use .canopyignore rules', async () => {
    ...
    expect(results).toEqual(['src/', '.canopyignore', 'src/not-ignored.js']);
  });

  it('should combine .gitignore and .canopyignore rules', async () => {
    tmpDir = await createTmpDir({
      '.gitignore': 'dist/',
      '.canopyignore': 'build/',
