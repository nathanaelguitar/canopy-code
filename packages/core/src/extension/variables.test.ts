...
describe('substituteHookVariables', () => {
  ...
  it('should handle multiple event types with hooks', () => {
    const basePath = '/home/user/.canopy/extensions/my-extension';
    ...
    expect(
      (result!['PreToolUse']![0].hooks![0] as { command: string }).command,
    ).toBe('/home/user/.canopy/extensions/my-extension/scripts/pre-start.sh');
    expect(result!['UserPromptSubmit']).toHaveLength(1);
    expect(
      (result!['UserPromptSubmit']![0].hooks![0] as { command: string })
        .command,
    ).toBe('/home/user/.canopy/extensions/my-extension/setup/install.py');
  });
...
describe('performVariableReplacement', () => {
  ...
  it('should replace .claude with .canopy in markdown files', () => {
    const extDir = path.join(testDir, 'ext');
    fs.mkdirSync(extDir, { recursive: true });
    ...
    expect(result).toContain('.canopy/loop.local.md');
    expect(result).toContain('rm .canopy/loop.local.md');
    expect(result).toContain('$HOME/.canopy/cache');
  ...
  it('should replace .claude with .canopy in shell scripts', () => {
    ...
    expect(result).toContain('./.canopy/local');
    expect(result).toContain('.canopy/config');
    // Note: URLs are also being replaced in current implementation
    expect(result).toContain('https://example.com/.canopy/page');
  });
