...
import { CanopyLogger, TEST_ONLY } from './canopy-logger.js';
...
const makeFakeConfig = (overrides: Partial<Config> = {}): Config => {
  const defaults = {
    ...
    getAuthType: () => AuthType.CANOPY_OAUTH,
    getMcpServers: () => ({}),
    getModel: () => 'test-model',
...
describe('CanopyLogger', () => {
  ...
  beforeEach(() => {
    ...
    (CanopyLogger as any).instance = undefined;
  });
  ...
  describe('getInstance', () => {
    it('returns undefined when usage statistics are disabled', () => {
      ...
      const logger = CanopyLogger.getInstance(config);
      expect(logger).toBeUndefined();
    });

    it('returns an instance when usage statistics are enabled', () => {
      const logger = CanopyLogger.getInstance(mockConfig);
      expect(logger).toBeInstanceOf(CanopyLogger);
    });

    it('is a singleton', () => {
      const logger1 = CanopyLogger.getInstance(mockConfig);
      const logger2 = CanopyLogger.getInstance(mockConfig);
  ...
  describe('logHookCallEvent', () => {
    ...
    it('should not include submitted prompts in hook telemetry', () => {
      ...
      const logger = CanopyLogger.getInstance(configWithLogPrompts)!;
      const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
    ...
    it('should not include error when telemetry log prompts disabled', () => {
      ...
      (CanopyLogger as unknown as { instance: undefined }).instance = undefined;
      const logger = CanopyLogger.getInstance(configWithoutLogPrompts)!;
      const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
    ...
    it('should sanitize hook name to remove sensitive information', () => {
      ...
      const event = new HookCallEvent(
        'PreToolUse',
        'command',
        '/home/user/.canopy/hooks/check-secrets.sh --api-key=secret123',
        { tool_name: 'read_file' },
        100,
        true,
      );
