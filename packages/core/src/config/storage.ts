...
import { getProjectHash, CANOPY_DIR, sanitizeCwd } from '../utils/paths.js';
import { FatalConfigError } from '../utils/errors.js';

export { CANOPY_DIR } from '../utils/paths.js';
export const GOOGLE_ACCOUNTS_FILENAME = 'google_accounts.json';
export const OAUTH_FILE = 'oauth_creds.json';
export const SKILL_PROVIDER_CONFIG_DIRS = ['.canopy', '.agents'];
...
export class Storage {
   ...
   * When null, falls back to getGlobalCanopyDir().
   */
   ...
   * Pass null/undefined/empty string to reset to default (getGlobalCanopyDir()).
   ...
   *              Pass the project root so that relative values like ".canopy" resolve
   ...
   *
   * Priority: pinned runtime context > CANOPY_RUNTIME_DIR env var > configurable context > setRuntimeBaseDir() value > getGlobalCanopyDir()
  ...
  static getRuntimeBaseDir(): string {
    ...
    if (contextualDir?.pinned) {
      return contextualDir.dir ?? Storage.getGlobalCanopyDir();
    }
    const envDir = process.env['CANOPY_RUNTIME_DIR'];
    if (envDir) {
      return (
        Storage.resolveRuntimeBaseDir(envDir) ?? Storage.getGlobalCanopyDir()
      );
    }
    ...
    return Storage.getGlobalCanopyDir();
  }

  static getGlobalCanopyDir(): string {
    ...
    if (!homeDir) {
      return path.join(os.tmpdir(), '.canopy');
    }
    return path.join(homeDir, CANOPY_DIR);
  }

  static getMcpOAuthTokensPath(): string {
    return path.join(Storage.getGlobalCanopyDir(), 'mcp-oauth-tokens.json');
  }

  static getGlobalSettingsPath(): string {
    return path.join(Storage.getGlobalCanopyDir(), 'settings.json');
  }

  static getInstallationIdPath(): string {
    return path.join(Storage.getGlobalCanopyDir(), 'installation_id');
  }

  static getGoogleAccountsPath(): string {
    return path.join(Storage.getGlobalCanopyDir(), GOOGLE_ACCOUNTS_FILENAME);
  }

  static getUserCommandsDir(): string {
    return path.join(Storage.getGlobalCanopyDir(), 'commands');
  }

  static getGlobalMemoryFilePath(): string {
    return path.join(Storage.getGlobalCanopyDir(), 'memory.md');
  }
  ...
  static getGlobalIdeDir(): string {
    // Pinned to the global Canopy dir so the VS Code companion (which only
    // sees env vars, not settings-based runtimeOutputDir) finds the same
    // lock-file location as the CLI.
    return path.join(Storage.getGlobalCanopyDir(), IDE_DIR_NAME);
  }
  ...
  static getPlansDir(
    ...
    return path.join(Storage.getGlobalCanopyDir(), PLANS_DIR_NAME);
  }
  ...
  static getGlobalBinDir(): string {
    return path.join(Storage.getGlobalCanopyDir(), BIN_DIR_NAME);
  }

  static getGlobalArenaDir(): string {
    return path.join(Storage.getGlobalCanopyDir(), ARENA_DIR_NAME);
  }

  getCanopyDir(): string {
    return path.join(this.targetDir, CANOPY_DIR);
  }
  ...
  static getOAuthCredsPath(): string {
    return path.join(Storage.getGlobalCanopyDir(), OAUTH_FILE);
  }
  ...
  getWorkspaceSettingsPath(): string {
    return path.join(this.getCanopyDir(), 'settings.json');
  }

  getProjectCommandsDir(): string {
    return path.join(this.getCanopyDir(), 'commands');
  }

  /**
   * Project-level saved-workflow scripts directory: `<targetDir>/.canopy/workflows`.
  ...
  getProjectWorkflowsDir(): string {
    return path.join(this.getCanopyDir(), 'workflows');
  }

  /**
   * User-level saved-workflow scripts directory: `~/.canopy/workflows`. User
   * scope is lower-precedence than project scope when the same `<name>.js`
   * exists in both.
   */
  static getUserWorkflowsDir(): string {
    return path.join(Storage.getGlobalCanopyDir(), 'workflows');
  }
  ...
  getExtensionsDir(): string {
    return path.join(this.getCanopyDir(), 'extensions');
  }

  getExtensionsConfigPath(): string {
    return path.join(this.getExtensionsDir(), 'canopy-extension.json');
  }
  ...
  /**
   * Returns the user-level extensions directory (~/.canopy/extensions/).
   * Extensions installed at user scope are stored here, as opposed to
   * project-level extensions which live in <project>/.canopy/extensions/.
   */
  static getUserExtensionsDir(): string {
    return path.join(Storage.getGlobalCanopyDir(), 'extensions');
  }
