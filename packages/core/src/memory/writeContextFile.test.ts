...
describe('writeWorkspaceContextFile', () => {
  ...
  let getGlobalCanopyDirSpy: ReturnType<typeof vi.spyOn>;

  beforeEach(async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'canopy-write-context-'));
  ...
  it('creates CANOPY.md with a fresh section header on first append', async () => {
  ...
  it('writes to the global ~/.canopy directory when scope=global', async () => {
    ...
    expect(getGlobalCanopyDirSpy).toHaveBeenCalled();
    const written = await fs.readFile(result.filePath, 'utf8');
    expect(written).toBe(`${MEMORY_SECTION_HEADER}\n- global entry\n`);
  });
  ...
  it('does not split a memory entry that contains `## ` inside a fenced code block', async () => {
    ...
    // (common in CANOPY.md memory entries that quote API docs with
  ...
  it('honors setGeminiMdFilename overrides so POST targets the same file GET surfaces', async () => {
    ...
    // but POST keep writing to `CANOPY.md`. The fix routes
    ...
    try {
      ...
      // The legacy CANOPY.md must NOT have been written — the prior
